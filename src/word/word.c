...
/*   Updated: 2025/05/22 16:31:12 by amakinen         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/*
	...
	fields to the given append pointer. Increment the num_fields counter by the
	number of fields appended.

	In case of an error, fields may be allocated and appended, and must be freed
	by the caller, but their content is undefined. The counter is only updated
	on success.
*/
...
/*
	...
	is overwritten via the append pointer. This unlinks the pattern field, but
	we must re-link the next field to prevent leaking the rest of the list in
...
/*
	Allocate memory for the field and add it to the linked list.

	The num_fields overflow check is impossible to reach in practice with 64-bit
	size_t, but do it anyway for maximum correctness.
*/
static t_status	word_out_allocate_field(struct s_word_state *state)
{
	struct s_word_field	*new_field;

	if (state->num_fields == SIZE_MAX)
		return (status_err(S_RESET_ERR, "word_expand", "too many fields", 0));
	state->num_fields++;
...
t_status	word_out_split(struct s_word_state *state)
{
	...
	if (state->out_idx == 0 && !state->out_has_quotes)
		return (S_OK);
	if (!state->out)
		status = word_out_allocate_field(state);
	else
